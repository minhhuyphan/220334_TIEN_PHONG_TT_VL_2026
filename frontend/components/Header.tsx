...
const Header: React.FC<HeaderProps> = ({ user, onMenuClick, hideMenuButton }) => {
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-10">
      <div className="flex items-center gap-3">
        ...
        <a href="/" className="flex items-center gap-2">
           ...
           <span className="font-bold text-indigo-600 text-sm">Zephyr</span>
        </a>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-6">
        {user ? (
          ...
          <a 
            href="/#/login" 
            className="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-full hover:bg-indigo-700 transition-colors"
          >
            Đăng nhập
          </a>
        )}
